		...
		foreach( MeshInstance combine in combines )
		{
			if (combine.mesh)
			{
				...
				if (generateStrips)
				{
					...
					int[] curTriangles = combine.mesh.GetTriangles(combine.subMeshIndex);
					int curStripCount = curTriangles.Length;
					if (curStripCount != 0)
					{
						if( stripCount != 0 )
						{
				...
				if (generateStrips)
				{
					...
					if (stripOffset != 0)
					{
						if ((stripOffset & 1) == 1)
						{
							...
							strip[stripOffset+1] = inputTriangles[0] + vertexOffset;
							strip[stripOffset+2] = inputTriangles[0] + vertexOffset;
							stripOffset+=3;
						}
						else
						{
					...
					{
						strip[i+stripOffset] = inputTriangles[i] + vertexOffset;
					}
					stripOffset += inputTriangles.Length;
				}
				else
				{
					int[]  inputTriangles = combine.mesh.GetTriangles(combine.subMeshIndex);
