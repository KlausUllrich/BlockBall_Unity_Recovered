...
- **Slope Physics Problem (Resolved on 2025-06-13)**: Fixed issue where ball would not accelerate on slopes in any physics mode due to excessive friction. Root cause was multiple friction layers: Physics materials (static=0.8, dynamic=0.5), PhysicsSettings rollingFriction=0.8, and additional collision friction. Solution reduced friction values: Block/Blockball materials now static=0.2/dynamic=0.3/bounciness=0.1, PhysicsSettings rollingFriction=0.3/slidingFriction=0.1. Ball now rolls naturally on slopes while maintaining gameplay control across all three physics modes. Files modified: `Block.physicMaterial`, `Blockball.physicMaterial`, `PhysicsSettings.cs`.
- **Slope Physics Problem - NEW ROOT CAUSE DISCOVERED (2025-06-13)**: Initial friction reduction fix failed testing. Ball still does not roll on slopes. Debug analysis revealed `Rigidbody.UseGravity=False` - Unity gravity is completely disabled! Root cause: `PhysicObject.cs` line 35 `UseUnityGravity = false` forces custom gravity system. Even in UnityPhysics mode, ball uses custom gravity which may not work properly on slopes. **Status: DEBUGGING TOOLS CREATED, GRAVITY FIX PENDING**. New debug scripts: `PhysicsDebugUI.cs` (on-screen monitoring), `GravityFixForSlopeTesting.cs` (temporary gravity fix). Next: Enable Unity gravity and re-test slope physics.
